...
class TotalDerivTestsFlorisAEPOpt(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # set up problem
        # prob = om.Problem(model=OptAEP(nTurbines, nDirections=1))
        ...
        prob = om.Problem(model=OptAEP(nTurbines=nTurbines, nDirections=windDirections.size, nVertices=nVertices,
        ...
        prob.model.add_design_var('turbineX', lower=np.ones(nTurbines)*min(turbineX), upper=np.ones(nTurbines)*max(turbineX), scaler=1.0)
        prob.model.add_design_var('turbineY', lower=np.ones(nTurbines)*min(turbineY), upper=np.ones(nTurbines)*max(turbineY), scaler=1.0)
        ...
        prob.model.add_constraint('boundaryDistances', lower=np.zeros(nVertices*nTurbines), scaler=1.0)
    ...
    def testObj(self):
        ...
        np.testing.assert_allclose(self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineX')]['J_fwd'], self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineX')]['J_fd'], self.rtol, self.atol)
        np.testing.assert_allclose(self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineY')]['J_fwd'], self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineY')]['J_fd'], self.rtol, self.atol)
        for dir in np.arange(0, self.nDirections):
            np.testing.assert_allclose(self.J[('obj_comp.obj', 'AEPgroup.y_ivc.yaw%i' % dir)]['J_fwd'], self.J[('obj_comp.obj', 'AEPgroup.y_ivc.yaw%i' % dir)]['J_fd'], self.rtol, self.atol)
...
class TotalDerivTestsFlorisAEPOptRotor(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        prob = om.Problem(model=OptAEP(nTurbines=nTurbines, nDirections=windDirections.size,
        ...
        # select design variables
        prob.model.add_design_var('turbineX', lower=np.ones(nTurbines)*min(turbineX), upper=np.ones(nTurbines)*max(turbineX), scaler=1E-2)
        prob.model.add_design_var('turbineY', lower=np.ones(nTurbines)*min(turbineY), upper=np.ones(nTurbines)*max(turbineY), scaler=1E-2)
        for direction_id in range(0, windDirections.size):
            prob.model.add_design_var('yaw%i' % direction_id, lower=-30.0, upper=30.0, scaler=1E-1)
    ...
    def testObj(self):
        ...
        np.testing.assert_allclose(self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineX')]['J_fwd'], self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineX')]['J_fd'],
                                    self.rtol, self.atol)
        np.testing.assert_allclose(self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineY')]['J_fwd'], self.J[('obj_comp.obj', 'AEPgroup.desvars.turbineY')]['J_fd'], self.rtol, self.atol)
        for dir in np.arange(0, self.nDirections):
            np.testing.assert_allclose(self.J[('obj_comp.obj', 'AEPgroup.y_ivc.yaw%i' % dir)]['J_fwd'], self.J[('obj_comp.obj', 'AEPgroup.y_ivc.yaw%i' % dir)]['J_fd'], self.rtol, self.atol)
...
class TotalDerivTestsGaussOpt_VestasV80(unittest.TestCase):

    @classmethod
    def setUpClass(self):

        super(TotalDerivTestsGaussOpt_VestasV80, self).setUpClass()
        nTurbines = 16
        nDirections = 50
        self.rtol = 1E-2
        self.atol = 1E5
        ...
        prob['rated_wind_speed'] = np.ones(nTurbines)*16.
        prob['cut_out_speed'] = 25.*np.ones(nTurbines)
...
class TotalDerivTestsGaussAEP_VestasV80(unittest.TestCase):

    @classmethod
    def setUpClass(self):

        super(TotalDerivTestsGaussAEP_VestasV80, self).setUpClass()
        nTurbines = 16
        nDirections = 50
        self.rtol = 1E-2
        self.atol = 1E5
...
class GradientTestsGauss(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        #
        # # set up problem
        # prob = om.Problem(model=AEPGroup(nTurbines, nDirections=1, use_rotor_components=False,
        #                              wake_model=gauss_wrapper, differentiable=True,
        #                              params_IdepVar_func=add_gauss_params_IndepVarComps,
        #                              wake_model_options={'nSamples': 0},
        #                              params_IdepVar_args=None))
        #
        # # initialize problem
        # prob.setup()
        #
        ...
        # initialize om.Problem
        prob = om.Problem(model=OptAEP(nTurbines=nTurbs, nDirections=windDirections.size, nVertices=1,
        ...
        bm = 4
        # select design variables
        prob.model.add_design_var('turbineX', lower=np.ones(nTurbs) * min(turbineX) * 0,
                              upper=np.ones(nTurbs) * max(turbineX) * bm, scaler=1)
        prob.model.add_design_var('turbineY', lower=np.ones(nTurbs) * min(turbineY) * 0,
                              upper=np.ones(nTurbs) * max(turbineY) * bm, scaler=1)
        for direction_id in range(0, windDirections.size):
            prob.model.add_design_var('yaw%i' % direction_id, lower=-30.0, upper=30.0, scaler=1)

        # add constraints
        # prob.model.add_constraint('sc', lower=np.zeros(int(((nTurbs - 1.) * nTurbs / 2.))), scaler=1.0)
    ...
    def testGaussGrads_wtVelocity0_turbineXw(self):
        np.testing.assert_allclose(self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'turbineXw')]['J_fwd'],
                                   self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'turbineXw')]['J_fd'], self.rtol_p, self.atol_p)

    def testGaussGrads_wtVelocity0_turbineYw(self):
        np.testing.assert_allclose(self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'turbineYw')]['J_fwd'],
                                   self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'turbineYw')]['J_fd'], self.rtol_p, self.atol_p)

    def testGaussGrads_wtVelocity0_rotorDiameter(self):
        np.testing.assert_allclose(self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'rotorDiameter')]['J_fwd'],
                                   self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'rotorDiameter')]['J_fd'], self.rtol_p, self.atol_p)

    def testGaussGrads_wtVelocity0_yaw0(self):
        np.testing.assert_allclose(self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'yaw0')]['J_fwd'],
                                   self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'yaw0')]['J_fd'], self.rtol_p, self.atol_p)

    def testGaussGrads_wtVelocity0_hubHeight(self):
        np.testing.assert_allclose(self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'hubHeight')]['J_fwd'],
                                   self.J['AEPgroup.all_directions.direction_group0.myModel.f_1'][('wtVelocity0', 'hubHeight')]['J_fd'], self.rtol_p, self.atol_p)

    def testObj_x(self):
        np.testing.assert_allclose(self.Jt[('obj_comp.obj', 'AEPgroup.desvars.turbineX')]['J_fwd'], self.Jt[('obj_comp.obj', 'AEPgroup.desvars.turbineX')]['J_fd'], self.rtol_t, self.atol_t)

    def testObj_y(self):
        np.testing.assert_allclose(self.Jt[('obj_comp.obj', 'AEPgroup.desvars.turbineY')]['J_fwd'], self.Jt[('obj_comp.obj', 'AEPgroup.desvars.turbineY')]['J_fd'],
                                   self.rtol_t, self.atol_t)

    def testObj_yaw(self):
        for dir in np.arange(0, self.nDirections):
            np.testing.assert_allclose(self.Jt[('obj_comp.obj', 'AEPgroup.y_ivc.yaw%i' % dir)]['J_fwd'],
                                       self.Jt[('obj_comp.obj', 'AEPgroup.y_ivc.yaw%i' % dir)]['J_fd'], self.rtol_t, self.atol_t)
...
class GradientTestsCtCpSingleValue(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # set up problem
        prob = om.Problem(model=AEPGroup(nTurbines=nTurbines, use_rotor_components=False))
...
class GradientTestsCpArray(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # set up problem
        prob = om.Problem(model=AEPGroup(nTurbines=nTurbines, use_rotor_components=False, cp_points=cp_points))
...
class GradientTestsCtCpRotor(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # set up problem
        prob = om.Problem(model=AEPGroup(nTurbines=nTurbines, use_rotor_components=use_rotor_components, datasize=datasize))
...
class GradientTestsPowerCP(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        turbineX = np.random.rand(nTurbines)*300.
        turbineY = np.random.rand(nTurbines)*300.
        ...
        # set up problem
        prob = om.Problem(model=AEPGroup(nTurbines=nTurbines, use_rotor_components=False, wake_model=gauss_wrapper,
    ...
    def testPower_wtPower_high(self):
        ...
        prob.run_model()
        J = prob.check_partials(out_stream=None)
        np.testing.assert_allclose(J['all_directions.direction_group0.powerComp'][('wtPower0', 'wtVelocity0')]['J_fwd'],
                                   J['all_directions.direction_group0.powerComp'][('wtPower0', 'wtVelocity0')]['J_fd'],
                                   self.rtol, self.atol)
        np.testing.assert_allclose(J['all_directions.direction_group0.powerComp'][('wtPower0', 'Cp')]['J_fwd'],
                                   J['all_directions.direction_group0.powerComp'][('wtPower0', 'Cp')]['J_fd'],
                                   self.rtol, self.atol)
        np.testing.assert_allclose(
            J['all_directions.direction_group0.powerComp'][('wtPower0', 'rotorDiameter')]['J_fwd'],
            J['all_directions.direction_group0.powerComp'][('wtPower0', 'rotorDiameter')]['J_fd'], self.rtol, self.atol)
    ...
    def testPower_totalpower_high(self):
        ...
        J = prob.check_partials(out_stream=None)
        np.testing.assert_allclose(
            J['all_directions.direction_group0.powerComp'][('dir_power0', 'wtVelocity0')]['J_fwd'],
            J['all_directions.direction_group0.powerComp'][('dir_power0', 'wtVelocity0')]['J_fd'], self.rtol, self.atol)
        np.testing.assert_allclose(J['all_directions.direction_group0.powerComp'][('dir_power0', 'Cp')]['J_fwd'],
                                   J['all_directions.direction_group0.powerComp'][('dir_power0', 'Cp')]['J_fd'],
                                   self.rtol, self.atol)
        np.testing.assert_allclose(
            J['all_directions.direction_group0.powerComp'][('dir_power0', 'rotorDiameter')]['J_fwd'],
            J['all_directions.direction_group0.powerComp'][('dir_power0', 'rotorDiameter')]['J_fd'], self.rtol,
            self.atol)
...
class GradientTestsPowerCurveDefinition(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        prob = om.Problem(model=AEPGroup(nTurbines=nTurbines, use_rotor_components=False, wake_model=gauss_wrapper, nDirections=10))
...
class GradientTestsConstraintComponents(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        prob_polygon = om.Problem(model=OptAEP(nTurbines=nTurbines, nDirections=windDirections.size, nVertices=nVertices,
                                               minSpacing=minSpacing, use_rotor_components=False, wake_model=gauss_wrapper,
                                               params_IdepVar_func=add_gauss_params_IndepVarComps,
                                               wake_model_options=wake_model_options, params_IdepVar_args=None))

        prob_circle = om.Problem(model=OptAEP(nTurbines=nTurbines, nDirections=windDirections.size, nVertices=1,
                                              minSpacing=minSpacing, use_rotor_components=False, wake_model=gauss_wrapper,
                                              params_IdepVar_func=add_gauss_params_IndepVarComps,
                                              wake_model_options=wake_model_options, params_IdepVar_args=None))
